...
class Captcha
{
    ...
    protected function generateCode(): array
    {
        ...
        if ($this->type == "1") {
            ...
            $opnum1  = rand(0, 9);
            $opnum2  = rand(0, 9);
            [$opnum1, $opnum2] = ($opindex == 1 ? [max($opnum1, $opnum2), min($opnum1, $opnum2)] : [$opnum1, $opnum2]);
            $code   = $opnum1 . $operate[$opindex] . $opnum2 . '=?';
            $answer = $opindex == 1 ? ($opnum1 - $opnum2) : ($opnum1 + $opnum2);
        }
        return [$code, (string)$answer];
    }
    ...
    protected function gdDrawText($image, $text)
    {
        ...
        imagettftext($image, $this->height * 0.8, 3, (int)round($this->width * 0.05 + 1), (int)round($this->height * 0.8 + 2), $color, $this->font, $code);
        //实际验证码
        $color = imagecolorallocate($image, rand(30, 120), rand(30, 120), rand(30, 120));
        imagettftext($image, $this->height * 0.6, 3, (int)round($this->width * 0.05), (int)round($this->height * 0.8), $color, $this->font, $text);
    }
